...
namespace CMSApi.Controllers
{
    ...
    public class FileController : ApiController
    {
        ...
        [Route("FileList")]
        public IHttpActionResult FileList()
        {
            ...
            foreach(var doc in documentItemViewModel)
            {
                using (var mgr = new CMSLcLy.Data.File.Manager())
                {
                    ...
                    var totalCount = WFItem.Count();
                    var totalNoComplete = WFItem.Where(x => x.isCompleted == 0).Count();

                    doc.ProgressStatus = ((totalNoComplete / totalCount) * 100).ToString();
                }
            }
        ...
        [Route("FileProgress")]
        public IHttpActionResult FileProgress(string FileName)
        {

            IList<WFMasterItemViewModel> wFMasterItemViewModel = null;

            using (var mgr = new CMSLcLy.Data.File.Manager())
            {
                wFMasterItemViewModel = mgr.ListByFileNo(FileName).ToList();
            }

            if (wFMasterItemViewModel == null) return NotFound();

            return Ok(wFMasterItemViewModel);
        }
